...
import { CONFIG_FILE, DEFAULT_PLATFORMS, DEFAULT_PROVIDER, DEFAULT_REPLY_MODE, DEFAULT_WEB_SEARCH, getNestedValue, loadExistingConfig, loadSchemas, PLATFORMS_DIR, PLUGINS_DIR, PROVIDERS_DIR, saveConfig, scanModules, setNestedValue, WEB_SEARCH_DIR, } from "../utils.js";
export default class Interactive extends Command {
    static description = "Interactively configure and launch Missy";
    static examples = [
    ...
    async run() {
        ...
        if (hasExisting) {
            ...
            this.log(`  ${chalk.cyan("Plugins:")}     ${(existing.plugins ?? []).join(", ") || "(none)"}`);
            this.log(`  ${chalk.cyan("Platforms:")}   ${(existing.platforms ?? []).join(", ") || "(none)"}`);
            this.log(`  ${chalk.cyan("Web Search:")} ${(existing.webSearch?.providerNames ?? []).join(", ") || "(none)"}`);
            this.log("");
        ...
        selectedWebSearch = availableWebSearch.length > 0
            ? await checkbox({
                ...
                choices: availableWebSearch.map((p) => ({
                    name: p,
                    value: p,
                    checked: Array.isArray(existing.webSearch?.providerNames)
                        ? existing.webSearch.providerNames.includes(p)
        ...
        setNestedValue(config, "replyMode", existing.replyMode ?? DEFAULT_REPLY_MODE);
    ...
    async promptField(field, existingValue) {
